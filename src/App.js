...
import QuoteWrap from './QuoteBox/QuoteWrap.js';
...
class App extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      tickers: localStorage.getItem('LS_tickers') ? JSON.parse(localStorage.getItem('LS_tickers')) : [],
  ...
  isValidTicker(ticker, callback){
    ...
    this.setState({
      tickerTextbox: { value: this.state.tickerTextbox.value, readOnly: true, style: 'tickerInput tickerInputLoading' },
  ...
  handleChange(event) {
    this.setState({
      tickerTextbox: { value: event.target.value.toUpperCase(), readOnly: false, style: 'tickerInput' }
    });
  }
    ...
    this.isValidTicker(this.state.tickerTextbox.value, (isValid) => {
      if(isValid){
        ...
        tickerArr.push({
          ...
          key: localStorage.getItem('LS_count') ? localStorage.getItem('LS_count') : 0
        });
        ...
        this.setState({
          tickerTextbox: { value: this.state.tickerTextbox.value, readOnly: false, style: 'tickerInput' },
